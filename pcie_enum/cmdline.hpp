...
/* usage
  cmdline::parser cmd;
  cmd.add<string>("host", 'h', "host name", true, "");
  cmd.add<int>("port", 'p', "port number", false, 80, cmdline::range(1, 65535));
  cmd.add<string>("type", 't', "protocol type", false, "http", cmdline::oneof<string>({"http", "https", "ssh", "ftp"}));
  cmd.add("gzip", 'g', "gzip when transfer"); //bool

  cmd.parse_check(argc, argv);

  cout << cmd.get<string>("type") << "://"
       << cmd.get<string>("host") << ":"
       << cmd.get<int>("port") << endl;
  if (cmd.exist("gzip")) cout << "gzip" << endl;
*/
...
template <typename T>
oneof_reader<T> oneof(std::initializer_list<T> values)
{
    oneof_reader<T> ret;
    for (auto& val : values) {
        ret.add(val);
